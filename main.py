...
class SmartTTSPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # Check reinstall switches
        if self.conf.get("reinstall_edge_tts", False):
            self._reinstall_edge_tts()

        if self.conf.get("reinstall_pilk", False):
            self._reinstall_pilk()
    ...
    def _reinstall_pilk(self):
        ...
        # Try direct install
        try:
            subprocess.run([pip_path, "install", "pilk"], capture_output=True, text=True, timeout=120)
        ...
        if not pilk_installed:
            ...
            try:
                r = subprocess.run([pip_path, "install", "pysilk"], capture_output=True, text=True, timeout=60)
                if r.returncode == 0:
